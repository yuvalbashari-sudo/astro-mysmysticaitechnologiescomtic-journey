...
const AstrologerIntroModal = ({ isOpen, onClose }: Props) => {
  ...
  const renderMarkdown = (text: string) => {
    ...
    lines.forEach((line, i) => {
      ...
      if (trimmed.startsWith("### ")) { elements.push(<h4 key={i} className="font-heading text-sm text-gold mt-3 mb-1">{renderInline(trimmed.slice(4))}</h4>); return; }
      if (trimmed.startsWith("## ")) { elements.push(<h3 key={i} className="font-heading text-[15px] text-gold mt-3 mb-1">{renderInline(trimmed.slice(3))}</h3>); return; }
      if (/^[-•*]\s/.test(trimmed)) { elements.push(<div key={i} className="flex gap-2 items-start"><span className="text-gold/50 mt-0.5 flex-shrink-0">•</span><span>{renderInline(trimmed.replace(/^[-•*]\s/, ""))}</span></div>); return; }
      if (/^\d+[.)]\s/.test(trimmed)) { const num = trimmed.match(/^(\d+)[.)]\s/)?.[1]; elements.push(<div key={i} className="flex gap-2 items-start"><span className="text-gold/50 mt-0.5 flex-shrink-0 text-xs min-w-[1rem] text-center">{num}.</span><span>{renderInline(trimmed.replace(/^\d+[.)]\s/, ""))}</span></div>); return; }
  ...
  return (
    <CinematicModalShell isOpen={isOpen} onClose={onClose} fullscreen>
      ...
      <div className="absolute inset-0 z-[102] flex" dir={dir}>
        ...
        <motion.div
          className="hidden md:flex flex-col justify-center px-10 pt-[110px] pb-10"
        ...
        <motion.div
          className="flex flex-col w-full md:w-[380px] md:max-w-[380px] md:min-w-[320px] h-full"
          style={{
        ...
        >
          ...
          >
            {messages.length === 0 && (
              <div className="text-center py-12 space-y-3">
                ...
                {/* Mobile suggestions */}
                <div className="flex flex-wrap gap-2 justify-center mt-3 md:hidden">
                  {suggestions.filter(Boolean).slice(0, 3).map((s, i) => (
                    <button
                      key={i}
                      onClick={() => { setInput(s); inputRef.current?.focus(); }}
                      className="text-xs px-3 py-1.5 rounded-full font-body transition-all hover:scale-105"
                      style={{
                        background: "hsl(var(--gold) / 0.06)",
                        border: "1px solid hsl(var(--gold) / 0.12)",
                        color: "hsl(var(--gold) / 0.6)",
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-3 font-body text-sm leading-relaxed ${msg.role === "user" ? "rounded-br-md" : "rounded-bl-md"}`}
                  style={msg.role === "user"
            ...
            {isLimitReached && !isStreaming && (
              ...
              >
                <div className="flex items-center justify-center gap-2">
                  ...
                  <p className="text-foreground/50 font-body text-xs leading-relaxed">{t.advisor_limit_reached}</p>
                </div>
                <button
                  className="text-xs px-4 py-2 rounded-full font-heading transition-all hover:scale-105"
                  style={{
                    background: "linear-gradient(135deg, hsl(var(--gold-dark)), hsl(var(--gold)))",
          ...
          {/* Input */}
          <div className="flex-shrink-0 px-4 py-3" style={{ borderTop: "1px solid hsl(var(--gold) / 0.08)" }}>
            <div
