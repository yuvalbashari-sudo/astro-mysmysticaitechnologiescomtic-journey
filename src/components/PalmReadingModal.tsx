...
const PalmReadingModal = ({ isOpen, onClose }: Props) => {
  ...
  const handleImageUpload = (side: "right" | "left") => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!file.type.startsWith("image/")) { toast("נא להעלות קובץ תמונה בלבד"); return; }
    if (file.size > 10 * 1024 * 1024) { toast("הקובץ גדול מדי — עד 10MB"); return; }
    const reader = new FileReader();
    reader.onload = () => {
      if (side === "right") setRightPalmImage(reader.result as string);
      else setLeftPalmImage(reader.result as string);
    };
  ...
  const handleSubmit = () => {
    if (!name.trim()) return;
    if (!rightPalmImage || !leftPalmImage) {
      toast("נא להעלות תמונות של שתי כפות הידיים");
      return;
    }
    setIsLoading(true);
  };
  ...
  const handleOnboardingComplete = () => {
    ...
    streamMysticalReading(
      "palm",
      { name: name.trim(), rightPalmImage, leftPalmImage },
      ...
      () => {
        ...
        readingsStorage.save({
          ...
          subtitle: "ניתוח שתי כפות הידיים",
          symbol: "✋",
  ...
  const handleClose = () => {
    onClose();
    setTimeout(() => {
      setSubmitted(false); setName(""); setRightPalmImage(null); setLeftPalmImage(null); setIsLoading(false);
  ...
  const handleShare = () => {
    const text = `✋ קריאת כף היד שלי\n\n🔮 גלו גם את קריאת כף היד שלכם:\n${window.location.origin}`;
    window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, "_blank");
  };

  const handleCopy = async () => {
    if (!aiText) return;
    await navigator.clipboard.writeText(`✋ קריאת כף היד — ${name}\n\n${aiText.slice(0, 300)}...`);
  ...
  const renderHandUpload = (side: "right" | "left", image: string | null, setImage: (v: string | null) => void, fileRef: React.RefObject<HTMLInputElement>, cameraRef: React.RefObject<HTMLInputElement>) => {
    const label = side === "right" ? "יד ימין — ההווה והפעולה" : "יד שמאל — הפוטנציאל והנשמה";
      ...
      <div className="flex-1 min-w-0">
        ...
        {image ? (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative rounded-xl overflow-hidden" style={{ border: "2px solid hsl(var(--gold) / 0.3)", boxShadow: "0 0 20px hsl(var(--gold) / 0.08)" }}>
            <img src={image} alt={`כף יד ${side === "right" ? "ימין" : "שמאל"}`} className="w-full h-40 object-cover" />
            <div className="absolute inset-0" style={{ background: "linear-gradient(to top, hsl(222 40% 8% / 0.4), transparent)" }} />
            <button onClick={() => setImage(null)} className="absolute top-2 left-2 w-6 h-6 rounded-full flex items-center justify-center bg-background/70 hover:bg-background/90 transition-colors" style={{ border: "1px solid hsl(var(--gold) / 0.2)" }}>
              <X className="w-3 h-3 text-gold/70" />
            </button>
            <div className="absolute bottom-2 right-2 px-2 py-0.5 rounded-full text-[9px] font-body" style={{ background: "hsl(142 70% 35% / 0.3)", border: "1px solid hsl(142 70% 45% / 0.4)", color: "hsl(142 70% 70%)" }}>
              <Check className="w-2.5 h-2.5 inline mr-0.5" />הועלתה
            </div>
          </motion.div>
        ) : (
          <motion.div className="rounded-xl p-5 cursor-pointer group transition-all duration-300" style={{ background: "linear-gradient(135deg, hsl(var(--gold) / 0.04), hsl(var(--gold) / 0.02))", border: "2px dashed hsl(var(--gold) / 0.2)" }} whileHover={{ scale: 1.01 }} onClick={() => fileRef.current?.click()}>
            <div className="flex flex-col items-center gap-2">
              <motion.div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "radial-gradient(circle, hsl(var(--gold) / 0.12), transparent)", border: "1px solid hsl(var(--gold) / 0.15)" }} animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 3, repeat: Infinity }}>
                <ImageIcon className="w-5 h-5 text-gold/60 group-hover:text-gold/80 transition-colors" />
              </motion.div>
              <p className="text-foreground/50 font-body text-[11px]">לחצו להעלאה</p>
            </div>
          </motion.div>
        )}
        ...
        {!image && (
          <div className="flex items-center justify-center gap-2 mt-2">
            <motion.button onClick={() => cameraRef.current?.click()} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-body" style={{ background: "hsl(var(--gold) / 0.08)", border: "1px solid hsl(var(--gold) / 0.15)", color: "hsl(var(--gold))" }} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
              <Camera className="w-3 h-3" />צלמו
            </motion.button>
            <motion.button onClick={() => fileRef.current?.click()} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-body" style={{ background: "hsl(var(--foreground) / 0.04)", border: "1px solid hsl(var(--foreground) / 0.1)", color: "hsl(var(--foreground) / 0.6)" }} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
              <Upload className="w-3 h-3" />גלריה
            </motion.button>
          </div>
        )}
      </div>
            ...
            <AnimatePresence mode="wait">
              {!submitted && !isLoading ? (
                <motion.div key="input" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-8 md:p-12 text-center">
                  ...
                  <p className="text-foreground/70 font-body text-sm md:text-base mb-4 max-w-md mx-auto leading-relaxed">צלמו את שתי כפות הידיים שלכם וקבלו ניתוח מעמיק — יד ימין חושפת את ההווה והפעולה, יד שמאל חושפת את הפוטנציאל הנסתר.</p>
                  ...
                  <div className="max-w-lg mx-auto mb-8 rounded-xl p-4 text-right" style={{ background: "hsl(var(--gold) / 0.04)", border: "1px solid hsl(var(--gold) / 0.1)" }}>
                    <p className="text-gold/60 font-body text-[11px] font-semibold mb-2">✦ טיפים לתמונות מושלמות:</p>
                    <ul className="text-foreground/50 font-body text-[11px] space-y-1 leading-relaxed">
                      <li>• פרשו את כפות הידיים בתאורה טובה</li>
                      <li>• צלמו מקרוב כך שקווי כף היד ייראו בבירור</li>
                      <li>• יד ימין = מה שאתם עושים בפועל, הפעולה בעולם</li>
                      <li>• יד שמאל = הפוטנציאל הטבוע, הנשמה והמתנות הנולדות</li>
                    </ul>
                  </div>

                  <motion.button onClick={handleSubmit} disabled={!name.trim() || !rightPalmImage || !leftPalmImage} className="btn-gold font-body flex items-center justify-center gap-2 mx-auto disabled:opacity-40 disabled:cursor-not-allowed" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }}><Hand className="w-4 h-4" />קראו את כפות הידיים שלי</motion.button>
                  <p className="text-[11px] text-muted-foreground font-body mt-6">✦ ניתוח חזותי מיסטי של שתי כפות הידיים — בחינם ✦</p>
                </motion.div>
              ) : isLoading ? (
                ...
                <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-6 md:p-10">
                  <div className="text-center mb-8">
                    <motion.div className="text-5xl mb-3 flex items-center justify-center gap-2" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", delay: 0.2 }}>
                      <span>🤚</span><span className="text-gold text-2xl">✦</span><span>✋</span>
                    </motion.div>
                    ...
                    <motion.p className="text-foreground/50 font-body text-xs mt-1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>ניתוח משולב של שתי כפות הידיים</motion.p>
                  ...
                  {aiText ? (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      ...
                      {aiLoading && (
                        <motion.div className="flex items-center justify-center gap-2 mt-6" animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 2, repeat: Infinity }}>
                          ...
                          <span className="font-body text-xs text-gold/50">מנתח את קווי כפות הידיים...</span>
                        </motion.div>
                      )}
                    </motion.div>
                  ) : aiError ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-12">
                      ...
                      <motion.p className="font-body text-gold/70 text-sm" animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 2, repeat: Infinity }}>מנתח את תמונות כפות הידיים שלכם...</motion.p>
                    </div>
                  )}
