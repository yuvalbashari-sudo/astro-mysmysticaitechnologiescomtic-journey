...
const TarotModal = ({ isOpen, onClose }: Props) => {
  ...
  const cardSections = [
    { key: "meaning", label: t.tarot_meaning, icon: Eye },
    { key: "love", label: t.tarot_love, icon: Heart },
    { key: "career", label: t.tarot_career, icon: Briefcase },
    { key: "spiritual", label: t.tarot_spiritual, icon: Sparkles },
    { key: "advice", label: t.tarot_advice, icon: Lightbulb },
  ] as const;
  ...
  const handleOnboardingComplete = () => {
    ...
    setCards(drawn); setIsLoading(false);
    readingsStorage.save({ type: "tarot", title: t.readings_type_tarot, subtitle: drawn.map(c => c.hebrewName).join(" • "), symbol: "🔮", data: { cards: drawn } });
  };
  ...
  const handleCombinedReading = () => {
    if (!cards || isCombinedLoading || combinedReading) { setShowCombined(true); return; }
    setShowCombined(true); setIsCombinedLoading(true);
    streamMysticalReading("tarotSpread",
      { card1Name: cards[0].name, card1Hebrew: cards[0].hebrewName, card2Name: cards[1].name, card2Hebrew: cards[1].hebrewName, card3Name: cards[2].name, card3Hebrew: cards[2].hebrewName },
            ...
            <AnimatePresence mode="wait">
              {!cards && !isLoading ? (
                ...
                <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-6 md:p-10">
                    ...
                    <div className="flex items-center justify-center gap-4 mb-6">
                      {cards.map((card, i) => (
                        <motion.button key={i} onClick={() => setActiveCard(i)} className={`flex flex-col items-center gap-1 px-4 py-3 rounded-xl transition-all ${activeCard === i ? "ring-1 ring-gold/40" : ""}`} style={{ background: activeCard === i ? "hsl(var(--gold) / 0.1)" : "hsl(var(--deep-blue-light) / 0.3)", border: `1px solid hsl(var(--gold) / ${activeCard === i ? "0.3" : "0.08"})` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.2 }} whileHover={{ scale: 1.05 }}>
                          {tarotCardImages[card.name] ? <img src={tarotCardImages[card.name]} alt={card.hebrewName} className="w-10 h-14 object-cover rounded" /> : <span className="text-2xl">{card.symbol}</span>}
                          <span className={`font-body text-xs ${activeCard === i ? "text-gold" : "text-foreground/60"}`}>{card.hebrewName}</span>
                          <span className="text-[10px] text-muted-foreground font-body">{t.tarot_card_label} {i + 1}</span>
                        </motion.button>
                      ))}
                    </div>
                  ...
                  <AnimatePresence mode="wait">
                    <motion.div key={activeCard} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                      <div className="text-center mb-4">
                        {tarotCardImages[cards[activeCard].name] ? <img src={tarotCardImages[cards[activeCard].name]} alt={cards[activeCard].hebrewName} className="w-32 h-48 object-cover rounded-lg mx-auto shadow-lg" style={{ border: "2px solid hsl(var(--gold) / 0.3)" }} /> : <span className="text-4xl">{cards[activeCard].symbol}</span>}
                  ...
                  <ShareResultSection symbol={cards[activeCard].symbol} title={cards[activeCard].hebrewName} subtitle={t.readings_type_tarot} />
                  ...
                  {!showCombined ? (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }} className="text-center">
                      <motion.button onClick={handleCombinedReading} className="btn-gold font-body flex items-center justify-center gap-2 mx-auto" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }}><Layers className="w-4 h-4" />{t.tarot_combined_cta}</motion.button>
                      <p className="text-[11px] text-muted-foreground font-body mt-3">{t.tarot_combined_subtitle}</p>
                    </motion.div>
                  ) : (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl p-6 md:p-8" style={{ background: "linear-gradient(145deg, hsl(var(--deep-blue-light) / 0.6), hsl(var(--deep-blue) / 0.4))", border: "1px solid hsl(var(--gold) / 0.15)", boxShadow: "0 0 40px hsl(var(--gold) / 0.05)" }}>
                      <div className="flex items-center justify-center gap-3 mb-6"><Layers className="w-5 h-5 text-gold" /><h3 className="font-heading text-lg gold-gradient-text">{t.tarot_combined_title}</h3></div>
                      <div className="flex items-center justify-center gap-3 mb-6">
                        {cards.map((card, i) => (
                          <div key={i} className="flex flex-col items-center gap-1">
                            {tarotCardImages[card.name] ? <img src={tarotCardImages[card.name]} alt={card.hebrewName} className="w-12 h-16 object-cover rounded" style={{ border: "1px solid hsl(var(--gold) / 0.2)" }} /> : <span className="text-xl">{card.symbol}</span>}
                      ...
                      {combinedReading && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} dir="rtl">{renderMysticalText(combinedReading)}</motion.div>}
                    </motion.div>
                  )}
                  <div className="section-divider max-w-[200px] mx-auto my-8" />
