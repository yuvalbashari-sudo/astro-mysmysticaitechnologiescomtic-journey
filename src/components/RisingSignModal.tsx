...
const sections = [
  { key: "risingDescription", label: "המזל העולה שלך", icon: Sparkles },
  { key: "firstImpression", label: "רושם ראשוני", icon: Eye },
  { key: "socialMask", label: "המסכה החברתית", icon: Shield },
  { key: "lifeApproach", label: "גישה לחיים", icon: Compass },
  { key: "hiddenStrength", label: "כוח נסתר", icon: Flame },
  { key: "spiritualPath", label: "נתיב רוחני", icon: Crown },
] as const;
...
const RisingSignModal = ({ isOpen, onClose }: Props) => {
  ...
  const handleShare = () => {
    if (!result) return;
    const text = `✨ המזל העולה שלי — ${result.hebrewName} ${result.symbol}\n\n${result.risingDescription.slice(0, 100)}...\n\n🔮 גלו גם את המזל העולה שלכם בחינם:\n${window.location.origin}`;
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div className="fixed inset-0 z-[100] flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          >
            <button onClick={handleClose} className="absolute top-4 left-4 z-20 w-8 h-8 rounded-full flex items-center justify-center bg-muted/30 hover:bg-muted/50 transition-colors" style={{ border: "1px solid hsl(var(--gold) / 0.15)" }}>
              <X className="w-4 h-4 text-gold/70" />
            </button>
            <div className="absolute top-4 right-4 z-20">
              <span className="px-3 py-1 rounded-full text-[10px] font-bold font-body tracking-wider" style={{ background: "linear-gradient(135deg, hsl(var(--gold) / 0.2), hsl(var(--gold) / 0.1))", border: "1px solid hsl(var(--gold) / 0.3)", color: "hsl(var(--gold))" }}>✦ חינם</span>
            </div>
            ...
            <AnimatePresence mode="wait">
              {!result && !isLoading ? (
                <motion.div key="input" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-8 md:p-12 text-center">
                  ...
                  <h2 className="font-heading text-2xl md:text-3xl gold-gradient-text mb-3">המזל העולה שלך</h2>
                  <p className="text-foreground/70 font-body text-sm md:text-base mb-8 max-w-md mx-auto leading-relaxed">גלו את המזל העולה שלכם לפי שעת הלידה. המזל העולה חושף את הפנים שאתם מציגים לעולם, את הרושם הראשוני שאתם יוצרים ואת הנתיב הרוחני שלכם.</p>
                  <div className="max-w-xs mx-auto mb-8">
                    <label className="block text-sm text-gold/70 font-body mb-2 text-right">שעת לידה</label>
                  ...
                  <motion.button onClick={handleSubmit} disabled={!birthTime} className="btn-gold font-body flex items-center justify-center gap-2 mx-auto disabled:opacity-40 disabled:cursor-not-allowed" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }}>
                    <Sparkles className="w-4 h-4" />גלו את המזל העולה שלי
                  </motion.button>
                </motion.div>
              ) : isLoading ? (
                ...
                <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-6 md:p-10">
                  <div className="text-center mb-8">
                    <motion.div className="text-5xl mb-3" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", delay: 0.2 }}>{result.symbol}</motion.div>
                    <motion.h2 className="font-heading text-2xl md:text-3xl gold-gradient-text mb-1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>{result.hebrewName}</motion.h2>
                    <motion.p className="text-muted-foreground font-body text-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>יסוד {result.element}</motion.p>
                    ...
                    {sections.map((section, i) => {
                      ...
                      return (
                        <motion.div key={section.key} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + i * 0.1 }} className="rounded-xl p-5 md:p-6" style={{ background: "linear-gradient(135deg, hsl(var(--deep-blue-light) / 0.5), hsl(var(--deep-blue) / 0.3))", border: "1px solid hsl(var(--gold) / 0.08)" }}>
                          ...
                          <p className="text-foreground/75 font-body text-sm leading-[1.85] text-right">{result[section.key as keyof RisingSignResult]}</p>
                        </motion.div>
                      );
                    })}
                  </div>
                  <div className="section-divider max-w-[200px] mx-auto my-8" />
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.5 }} className="text-center rounded-xl p-6" style={{ background: "linear-gradient(135deg, hsl(var(--crimson) / 0.08), hsl(var(--gold) / 0.05))", border: "1px solid hsl(var(--gold) / 0.12)" }}>
