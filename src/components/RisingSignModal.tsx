...
const RisingSignModal = ({ isOpen, onClose }: Props) => {
  ...
  const handleClose = () => {
    onClose();
    setTimeout(() => {
      setSignInfo(null); setBirthTime(""); setIsLoading(false);
  ...
  const handleCopy = async () => {
    if (!aiText) return;
    await navigator.clipboard.writeText(`✨ מזל עולה — ${signInfo?.name}\n\n${aiText.slice(0, 300)}...`);
              ...
              {!signInfo && !isLoading ? (
                ...
                <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-6 md:p-10">
                  <div className="text-center mb-8">
                    ...
                    <motion.p className="text-muted-foreground font-body text-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>יסוד {signInfo.element}</motion.p>
                  ...
                  {aiText ? (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      ...
                      {aiLoading && (
                        <motion.div className="flex items-center justify-center gap-2 mt-6" animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 2, repeat: Infinity }}>
                          ...
                          <span className="font-body text-xs text-gold/50">המזל העולה מתגלה...</span>
                        </motion.div>
                      )}
                    </motion.div>
                  ) : aiError ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-12">
                      ...
                      <motion.p className="font-body text-gold/70 text-sm" animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 2, repeat: Infinity }}>מגלים את המזל העולה שלכם...</motion.p>
                    </div>
                  )}

                  {!aiLoading && (aiText || aiError) && (
                    <>
                      <ShareResultSection symbol={signInfo.symbol} title={`מזל עולה — ${signInfo.name}`} subtitle={signInfo.element} />
