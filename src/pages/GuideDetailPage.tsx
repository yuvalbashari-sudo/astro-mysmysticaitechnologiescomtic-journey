        ...
        {guide.sections.map((section, i) => (
          ...
          >
            ...
            {section.blocks?.map((block, j) => (
              <p
                key={j}
                className="font-body text-sm text-foreground/75 leading-[2] mb-3 last:mb-0"
              >
                {block}
              </p>
            ))}
            ...
            {section.bullets && (
              <ul className="space-y-2 mt-2">
                {section.bullets.map((bullet, j) => (
                  <li
                    key={j}
                    className="flex items-start gap-2 font-body text-sm text-foreground/75 leading-relaxed"
                  >
