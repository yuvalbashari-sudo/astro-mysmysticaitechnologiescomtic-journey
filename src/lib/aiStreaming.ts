...
export function renderMysticalText(text: string): React.ReactNode {
  ...
  lines.forEach((line, i) => {
    ...
    if (trimmed === "---") {
      elements.push(React.createElement("div", { key: i, className: "section-divider max-w-[80px] mx-auto my-6" }));
      return;
    }
    if (trimmed.startsWith("### ✨") || trimmed.startsWith("### 🌟")) {
      elements.push(
        React.createElement("div", {
          ...
          React.createElement(Sparkles, { className: "w-5 h-5 text-gold mx-auto mb-2" }),
          React.createElement("h3", { className: "font-heading text-sm text-gold mb-2" }, trimmed.replace(/### [✨🌟]\s?/, ""))
        )
      );
      return;
    }
    if (trimmed.startsWith("### ")) {
      elements.push(
        React.createElement("div", { key: i, className: "flex items-center gap-3 mt-6 mb-3" },
          React.createElement("div", {
          ...
          }, React.createElement("span", { className: "text-lg" }, trimmed.match(/[\p{Emoji}]/u)?.[0] || "✦")),
          React.createElement("h3", { className: "font-heading text-base text-gold" }, trimmed.replace("### ", ""))
        )
      );
      return;
    }
    if (trimmed.startsWith("**") && trimmed.endsWith("**")) {
      ...
      elements.push(
        React.createElement("div", { key: i, className: "flex items-center gap-2 mt-4 mb-1" },
          React.createElement("div", {
            className: "w-6 h-6 rounded-full flex items-center justify-center",
            style: { background: "hsl(var(--gold) / 0.1)" },
          }, React.createElement("span", { className: "text-xs" }, label.match(/[\p{Emoji}]/u)?.[0] || "✦")),
          React.createElement("h4", { className: "font-heading text-xs text-gold" }, label.replace(/[\p{Emoji}]\s?/u, "").trim())
        )
      );
      return;
    }
    if (trimmed.startsWith("״") || trimmed.startsWith('"') || trimmed.startsWith("\"")) {
      elements.push(
        React.createElement("div", {
          key: i,
          className: "rounded-xl p-4 text-center mt-2 mb-2",
          style: { background: "hsl(var(--gold) / 0.04)", border: "1px solid hsl(var(--gold) / 0.1)" },
        }, React.createElement("p", { className: "text-gold/80 font-body text-sm leading-relaxed italic" }, trimmed))
      );
      return;
    }
    ...
    elements.push(
      React.createElement("p", { key: i, className: "text-foreground/70 font-body text-sm leading-[1.85] text-right" }, cleanText)
    );
  });

  return React.createElement("div", { className: "space-y-0" }, ...elements);
}
