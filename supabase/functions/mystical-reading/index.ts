...
const READING_PROMPTS: Record<string, (data: any) => { system: string; user: string | any[] }> = {
  forecast: (data) => {
    ...
    const genderInstruction = isMale
      ? `\n\nהנחיית מגדר קריטית — חובה לכל התחזית:\nהקורא הוא גבר. כתוב את כל התחזית בלשון זכר עקבית מתחילה ועד הסוף.\nדוגמאות: "אתה תחווה", "הכוחות שלך", "אתה מוזמן", "עליך לשים לב", "אתה עומד בפני".\nאסור בשום מקום להשתמש בלשון נקבה או בלשון ניטרלית/רבים. כל משפט, כל פסקה, כל כותרת — בלשון זכר.`
      : isFemale
      ? `\n\nהנחיית מגדר קריטית — חובה לכל התחזית:\nהקוראת היא אישה. כתוב את כל התחזית בלשון נקבה עקבית מתחילה ועד הסוף.\nדוגמאות: "את תחווי", "הכוחות שלך", "את מוזמנת", "עלייך לשים לב", "את עומדת בפני".\nאסור בשום מקום להשתמש בלשון זכר או בלשון ניטרלית/רבים. כל משפט, כל פסקה, כל כותרת — בלשון נקבה.`
      : "";
...
${data.gender ? `מגדר הקורא/ת: ${isMale ? 'זכר — כתוב בלשון זכר בלבד לאורך כל התחזית' : 'נקבה — כתוב בלשון נקבה בלבד לאורך כל התחזית'}` : ''}

התחזית חייבת להיות ייחודית, עמוקה ורגשית. התאם לחודש הנוכחי.${data.gender ? '\nחשוב: שמור על עקביות מגדרית מוחלטת בכל משפט מההתחלה ועד הסוף.' : ''}`,
    };
  },
